...
- (void)viewDidLoad {
  ...
  [[NSNotificationCenter defaultCenter]  addObserver:self selector:@selector(showDiscoveryView:) name:OOOoyalaPlayerPlayCompletedNotification object:self.playerViewController];
  [[NSNotificationCenter defaultCenter]  addObserver:self selector:@selector(updatetitleDiscovery:) name:OOOoyalaPlayerPlayStartedNotification object:self.playerViewController.ooyalaPlayer];
  ...
  self.carousels = [[NSMutableArray alloc] init];
  
  self.similartableview = [[NSMutableArray alloc] init];
...
- (void) UserRender{
  ...
  if (!UIDeviceOrientationIsLandscape([[UIDevice currentDevice] orientation])) {
   ...
   [self.view addSubview:self.descriptionLabel];
    ...
    [self.videoTitle setHidden:YES];   //remove title uilabel
    [self.descriptionLabel setHidden:YES]; //remove description label
...
-(void) updatetitleDiscovery:(NSNotification *) notification
{
  ...
  if (self.playerViewController.ooyalaPlayer.currentItem.title != nil){
...
-(UITableView *)makeTableView: (int)height valuewidth: (int)width ycoord:(int) y_coord
{
  CGFloat x = 0;
  CGFloat y = y_coord;
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  cell.thumbnail.image = nil;
    ...
    if([self.discoveryResults[0] count] > 1){
      ...
      NSString *name = [self decodeString:[dict objectForKey:@"name"]];
      ...
      cell.title = name;
      cell.embedcode = [dict objectForKey:@"embed_code"];
...
- (void)insertDiscoveryResults:(NSArray *)results toArray:(NSMutableArray *)array {
  ...
  for (NSDictionary *dict in results) {
    //Decoding HTML data
    NSData* stringData = [[dict objectForKey:@"name"] dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary* options = @{NSDocumentTypeDocumentAttribute:NSHTMLTextDocumentType};
    NSAttributedString* decodedAttributedString = [[NSAttributedString alloc] initWithData:stringData options:options documentAttributes:NULL error:NULL];
