...
func downloadNewRCPackages(config vc.Config, zipConfig zc.Config, l *log.Logger) {
	ol.ColorizedMethodPrintln(l)
	err := util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath, []string{"wget '" + zipConfig.CoreSDKURL + "' --progress=bar -O " + zipConfig.CoreSDKFileName.S}, l)
	util.MaybeDie(err, l)

	err = util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath, []string{"wget '" + zipConfig.FreewheelSDKURL + "' --progress=bar -O " + zipConfig.FreewheelSDKFileName.S}, l)
	util.MaybeDie(err, l)

	err = util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath,[]string{ "wget '" + zipConfig.IMASDKURL + "' --progress=bar -O " + zipConfig.IMASDKFileName.S}, l)
	util.MaybeDie(err, l)
}
