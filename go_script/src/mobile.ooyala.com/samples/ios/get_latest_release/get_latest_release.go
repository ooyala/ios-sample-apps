...
func downloadNewRCPackages(config vc.Config, zipConfig zc.Config, l *log.Logger) {
	ol.ColorizedMethodPrintln(l)
	err := util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath, []string{"wget '" + zipConfig.CoreSDKURL + "' --progress=bar -O " + zipConfig.CoreSDKFileNameStr}, l)
	util.MaybeDie(err, l)

	err = util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath, []string{"wget '" + zipConfig.FreewheelSDKURL + "' --progress=bar -O " + zipConfig.FreewheelSDKFileNameStr}, l)
	util.MaybeDie(err, l)

	err = util.RunBashCommandsInDir(config.VendorOoyalaRootFolderPath,[]string{ "wget '" + zipConfig.IMASDKURL + "' --progress=bar -O " + zipConfig.IMASDKFileNameStr}, l)
	util.MaybeDie(err, l)
}
