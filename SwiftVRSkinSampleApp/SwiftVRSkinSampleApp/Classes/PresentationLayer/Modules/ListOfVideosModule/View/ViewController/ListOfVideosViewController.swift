...
class ListOfVideosViewController: UIViewController {
  ...
  private func configureNavigationBar() {
    ...
    let customVideoBarButtonItem = UIBarButtonItem(barButtonSystemItem: .add,
                                                   target: self,
  ...
  private func showCustomVideoItemViewController() {
    let customVideoViewController = viewModel.configuredCustomVideoViewController { videoItem in
      self.openVideoPlayerModule(withVideoItem: videoItem)
    }

    present(customVideoViewController, animated: true, completion: nil)
  }

  private func openVideoPlayerModule(withVideoItem: VideoItem) {
    let viewController = viewModel.configuredVideoViewController(withVideoItem: withVideoItem,
                                                                 QAModeEnabled: QAModeSwitch.isOn)
...
extension ListOfVideosViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView,
    ...
    guard let cell =
      tableView.dequeueReusableCell(withIdentifier: "ListOfVideosViewControllerTableViewCellID") else {
