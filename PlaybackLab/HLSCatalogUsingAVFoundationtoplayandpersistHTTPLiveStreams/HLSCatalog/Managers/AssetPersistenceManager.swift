      ...
      let mediaCharacteristics = asset.availableMediaCharacteristicsWithMediaSelectionOptions
        ...
        for mediaCharacteristic in mediaCharacteristics {
            if let mediaSelectionGroup = asset.mediaSelectionGroup(forMediaCharacteristic: mediaCharacteristic) {
              ...
              print("savedOptions count: \(savedOptions.count)")
              for option in savedOptions {
                print("savedOptions option \(option), name: \(option.displayName)")
              }
              
              print("mediaSelectionGroup options count: \(mediaSelectionGroup.options.count)")
              for option in mediaSelectionGroup.options {
                print("mediaSelectionGroup option \(option), name: \(option.displayName)")
              }
...
extension AssetPersistenceManager: AVAssetDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: NSError?) {
        ...
        if let error = error {
            ...
            if mediaSelectionPair.mediaSelectionGroup != nil && mediaSelectionMap[task] != nil {
                /*
                ...
                let originalMediaSelection = mediaSelectionMap[task]
              ...
              print("mediaselectiongroup allos empty selection: \(mediaSelectionPair.mediaSelectionGroup?.allowsEmptySelection)")
                // Select the AVMediaSelectionOption in the AVMediaSelectionGroup we found earlier.
                mediaSelection.select(nil, in: mediaSelectionPair.mediaSelectionGroup!)
               ...
               options: AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: 2000000,
                 */
//                guard let task = assetDownloadURLSession.makeAssetDownloadTask(asset: task.urlAsset, assetTitle: asset.name, assetArtworkData: nil, options: [AVAssetDownloadTaskMediaSelectionKey: mediaSelection]) else { return }
//                
//                task.taskDescription = asset.name
//                
//                activeDownloadsMap[task] = asset
...
//                userInfo[Asset.Keys.downloadState] = Asset.DownloadState.downloaded.rawValue
    ...
    func urlSession(_ session: URLSession, assetDownloadTask: AVAssetDownloadTask, didFinishDownloadingTo location: URL) {
         ...
         */
      print("absolute path: \(location.absoluteString), relative path: \(location.relativePath)")
