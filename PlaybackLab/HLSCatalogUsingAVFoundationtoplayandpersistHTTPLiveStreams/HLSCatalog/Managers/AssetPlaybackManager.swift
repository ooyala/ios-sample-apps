...
class AssetPlaybackManager: NSObject {
    ...
    private var playerItem: AVPlayerItem? {
        ...
        didSet {
          ...
          for characteristc in asset.availableMediaCharacteristicsWithMediaSelectionOptions {
            if let group = asset.mediaSelectionGroup(forMediaCharacteristic: characteristc) {
              print("group \(group) allos empty media options: \(group.allowsEmptySelection)")
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: AnyObject?, change: [NSKeyValueChangeKey : AnyObject]?, context: UnsafeMutablePointer<Void>?) {
        ...
        if keyPath == #keyPath(AVURLAsset.isPlayable) {
          ...
          print("playerItem status: \(playerItem?.status.rawValue), offline: \(asset?.urlAsset.assetCache?.isPlayableOffline)")
